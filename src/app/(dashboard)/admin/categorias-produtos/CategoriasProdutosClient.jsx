...
import { Edit, Eye, Filter, Plus, RotateCcw, Search, Trash2 } from "lucide-react";
import Swal from "sweetalert2";
import { useCategoriasProdutos } from "@/hooks/useCategoriasProdutos.js";
import { inativarCategoriaProduto, reativarCategoriaProduto } from "@/services/categoriasProdutosService.js";
...
export default function CategoriasProdutosClient() {
    const { categorias, loading, page, setPage, totalPages, setSearch, statusFilter, setStatusFilter, sortColumn, sortDirection, handleSort, refrescarLista } = useCategoriasProdutos();
    ...
    const handleArchive = async (id, nome) => {
        ...
        try {
            await inativarCategoriaProduto(id);
            await Swal.fire({
                icon: "success",
                title: "Removida!",
    ...
    const handleReactivate = async (id, nome) => {
        const result = await Swal.fire({
            title: "Restaurar Categoria?",
            text: `"${nome}" voltará a ficar disponível.`,
            icon: "question",
            showCancelButton: true,
            confirmButtonColor: "#16a34a",
            cancelButtonColor: "#6b7280",
        ...
        try {
            await reativarCategoriaProduto(id);
            refrescarLista();
        }
        catch (error) {
            await Swal.fire({
    ...
    const columns = [
        ...
        },
        {
            ...
            accessor: "ativo",
            render: (_, item) => {
                const arquivado = item.deletado_em !==
                    null;
                const label = arquivado
                    ? "Arquivada"
                    : item.ativo
                        ? "Ativa"
                        : "Inativa";
                const background = arquivado
                    ? "#fee2e2"
                    : item.ativo
                        ? "#dcfce7"
                        : "#fef3c7";
                const color = arquivado
                    ? "#991b1b"
                    : item.ativo
                        ? "#166534"
                        : "#92400e";
