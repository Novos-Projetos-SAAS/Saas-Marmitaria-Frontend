...
import { Edit, Eye, Filter, Plus, RotateCcw, Search, Trash2, Tags } from "lucide-react";
import Swal from "sweetalert2";

import { useProdutos } from "@/hooks/useProdutos.js";
import { alterarDisponibilidadeProduto, inativarProduto, reativarProduto } from "@/services/produtosService.js";
import { buscarCategoriasProdutosParaSelect } from "@/services/categoriasProdutosService.js";
...
export default function ProdutosClient() {
    ...
    useEffect(() => {
        async function carregarCategorias() {
            try {
                const dados = await buscarCategoriasProdutosParaSelect();
                setCategorias(dados);
            } catch (error) {
     ...
     * Soft delete.
     */
    const handleArchiveProduto = async (id, nome) => {
        const result = await Swal.fire({
            title: "Remover Produto?",
            text: `O produto "${nome}" deixará de aparecer no cardápio.`,
            icon: "warning",
        ...
        try {
            ...
            Swal.fire({
                icon: "error",
                title: "Erro",
                text: error?.response?.data?.message || "Não foi possível remover o produto."
            });
        }
    };
     ...
     */
    const handleReactivateProduto = async (id, nome) => {
        const result = await Swal.fire({
            title: "Restaurar Produto?",
            text: `"${nome}" voltará ao cadastro, porém ficará indisponível até ser liberado.`,
            icon: "question",
            showCancelButton: true,
            confirmButtonColor: "#16a34a",
            cancelButtonColor: "#6b7280",
        ...
        try {
            ...
            await Swal.fire({
                title: "Restaurado!",
                text: "Produto restaurado. Libere-o quando quiser disponibilizá-lo.",
                icon: "success",
                confirmButtonColor: "#16a34a"
            });
            ...
            Swal.fire({
                icon: "error",
                title: "Erro",
                text: error?.response?.data?.message || "Não foi possível restaurar o produto."
            });
        }
    };
     ...
     */
    const handleDisponibilidade = async (produto) => {
        const novoStatus = !produto.disponivel_hoje;

        try {
            await alterarDisponibilidadeProduto(produto.id, novoStatus);
            refrescarLista();
        } catch (error) {
            Swal.fire({
                icon: "error",
                title: "Erro",
                text: error?.response?.data?.message || "Não foi possível alterar a disponibilidade."
            });
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value, item) => (
                <div>
                    <strong style={{ color: "#18181b", fontWeight: 600 }}>{value}</strong>
         ...
         */
        {
            header: "Preço",
            accessor: "preco",
            render: (value) => <strong style={{ color: "#18181b", fontWeight: 600 }}>{formatarMoeda(value)}</strong>
        },
        {
            header: "Status",
            accessor: "ativo",
            render: (_, item) => {
                const arquivado = item.deletado_em !== null;
                ...
                return (
                    <>
                        <div className={styles.desktopActions}>
                            <Can perform="produtos.visualizar">
                                <Link
                            ...
                            {!arquivado ? (
                                <Can perform="produtos.deletar">
                                    <button
                                        type="button"
                                        onClick={() => handleArchiveProduto(item.id, item.nome)}
                                        title="Remover"
                                ...
                                <Can perform="produtos.restaurar">
                                    <button
                                        type="button"
                                        onClick={() => handleReactivateProduto(item.id, item.nome)}
                                        title="Restaurar"
                        ...
                        <div className={styles.mobileActions}>
                            <ActionMenu
                                ...
                                onArchive={handleArchiveProduto}
                                onReactivate={handleReactivateProduto}
                                isLast={isLastItems}
                            />
                        </div>
                    </>
    ...
    return (
        <div className={styles.wrapper}>
            <div className={styles.actionsBar}>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                    <Can perform="categorias_produtos.listar">
                        <Link href="/admin/categorias-produtos" className={styles.newButton}>
                            <Tags size={19} />
                            <span>Categorias</span>
                        </Link>
                    </Can>

                    <Can perform="produtos.criar">
                        <Link href="/admin/produtos/cadastro" className={styles.newButton}>
                            <Plus size={20} />
            ...
            {!loading && produtos.length > 0 && totalPages > 1 && (
